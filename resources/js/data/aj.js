export const aj = {
  brand: 'aj',
  email: 'info@alejandrajaime.es',

  bioCorta:
    'Arte, marca personal y dirección creativa. Identidad, introspección y estética con significado.',

  bioLarga: [
    `Desde pequeña he tenido una curiosidad innata por los temas trascendentales de la vida, las cuestiones filosóficas y profundas. Por personalidad y rumbo de vida, irremediablemente he vivido procesos introspectivos hondos y me he sumergido en los fondos de la pregunta ¿quién soy?`,
    `Es parte natural de mi forma de vivir aprovechar los procesos dolorosos para transformarme, y se ha convertido en algo cíclico el plantearme cada poco mi identidad a todos los niveles, así que no vengo a concretar sobre mí demasiado, porque pronto esas definiciones me resultarán limitantes.`,
    `Sí me parece interesante contar algo importante para mí que viví en medio de esas transformaciones; el momento en el que descubrí más a fondo, estudiando moda, la magia de la creación. El impacto fue inmediato y sentí que mi vida por fin tenía sentido.`,
    `Sin embargo, al hacer de la moda mi herramienta también me supuso sentirme atrapada. Se convirtió en un escondite para mi ser, y me fui apegando a una identidad limitada y construida para el afuera, sin poder atender mi estado interno y mi bienestar. Cuando llegó el breakdown, pude conectar con muchos deseos del alma que habían estado acallados: fui soltando la rigidez identitaria con la que me había protegido para no ser dañada y ser querida.`,
    `Este proyecto es el resultado de escuchar el deseo de exploración, de abrirme a una nueva identidad personal y artística, que ahora quiere ser flexible y en continuo nacimiento, en conexión conmigo. Y a su vez, parte de la necesidad de ofrecer servicios enfocados en ese sentido, para que otros puedan lograr andar su propio camino de libertad, encontrar su autenticidad y ejercer la individuación propia.`,
  ],

  servicios: [
    {
      slug: 'mentoria-creativa',
      titulo: 'Mentoría creativa',
      subtitulo: 'Introspección creativa, ADN e identidad de marca',
      incluye: [
        'Exploración de propósito y narrativa',
        'Identidad conceptual y visual',
        'Dirección de arte y diseño de producto',
        'Acompañamiento creativo integral',
      ],
      cta: '/aj/contacto',
    },
    {
      slug: 'coaching-astrologico',
      titulo: 'Coaching astrológico',
      subtitulo: 'Lectura general de carta natal / consulta astrológica',
      incluye: [
        'Lectura de carta natal',
        'Tránsitos y ciclos',
        'Aplicación práctica al proceso personal/creativo',
      ],
      cta: '/aj/contacto',
    },
  ],

  portfolioCategorias: [
    { key: 'marca', label: 'María Magdalena (Marca)' },
    { key: 'screenplay', label: 'Screenplay' },
    { key: 'producto', label: 'Diseño de producto' },
    { key: 'videoclips', label: 'Colaboraciones en videoclips' },
    { key: 'estilismo', label: 'Estilismo y dirección de arte' },
    { key: 'otros', label: 'Otros' },
  ],

  proyectos: [
    // INTEGRACION
    {
      titulo: 'Integración — Fashion Film',
      categoria: 'marca',
      // Vimeo: usa el enlace público;
      media: ['https://vimeo.com/244922955'],
    },


    {
      titulo: 'Integración — Vogue',
      categoria: 'marca',
      media: [
        '/images/integracion/vogue/PF17_MariaMagdalena_009.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_001.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_002.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_003.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_004.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_005.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_006.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_007.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_008.webp',  
        '/images/integracion/vogue/PF17_MariaMagdalena_010.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_011.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_012.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_013.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_014.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_015.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_016.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_017.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_018.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_019.webp',
        '/images/integracion/vogue/PF17_MariaMagdalena_020.webp',
      ]
    },



    //WE ARE CATTLE
    // WE ARE CATTLE — Fashion Film
    {
      titulo: 'We are Cattle — Fashion Film',
      categoria: 'marca',
      media: ['https://vimeo.com/140413109'], // Vimeo autoplay
    },

    // WE ARE CATTLE — Vogue
    {
      titulo: 'We are Cattle — Vogue',
      categoria: 'marca',
      media: [
        
        '/images/we-are-cattle/vogue/00014-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00001-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00001-Maria-Magdalena-Madrid-Fall-2015-RTW-Details.jpg',
        '/images/we-are-cattle/vogue/00002-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00003-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00003-Maria-Magdalena-Madrid-Fall-2015-RTW-Details.jpg',
        '/images/we-are-cattle/vogue/00004-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00004-Maria-Magdalena-Madrid-Fall-2015-RTW-Details.jpg',
        '/images/we-are-cattle/vogue/00005-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00006-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00007-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00008-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00007-Maria-Magdalena-Madrid-Fall-2015-RTW-Details.jpg',
        '/images/we-are-cattle/vogue/00009-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00009-Maria-Magdalena-Madrid-Fall-2015-RTW-Details.jpg',
        '/images/we-are-cattle/vogue/00010-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00010-Maria-Magdalena-Madrid-Fall-2015-RTW-Details.jpg',
        '/images/we-are-cattle/vogue/00011-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00012-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00013-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00014-Maria-Magdalena-Madrid-Fall-2015-RTW-Details.jpg',
        '/images/we-are-cattle/vogue/00015-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00016-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00017-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00018-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00018-Maria-Magdalena-Madrid-Fall-2015-RTW-Details.jpg',
        '/images/we-are-cattle/vogue/00019-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00019-Maria-Magdalena-Madrid-Fall-2015-RTW-Details.jpg',
        '/images/we-are-cattle/vogue/00020-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00020-Maria-Magdalena-Madrid-Fall-2015-RTW-Details.jpg',
        '/images/we-are-cattle/vogue/00021-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00021-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00022-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
        '/images/we-are-cattle/vogue/00023-Maria-Magdalena-Madrid-Fall-2015-RTW.jpg',
      ],
    },

    // PRODUCTO
    {
      titulo: 'Play for Art (Adidas x Juancho Marqués)',
      categoria: 'producto',
      // Solo imágenes (ejemplo):
      media: [
        '/images/play-for-art/DSC05927.webp',
        '/images/play-for-art/DSC05813(1).webp',
        '/images/play-for-art/DSC05916.webp',
        '/images/play-for-art/DSC05920.webp', 
        '/images/play-for-art/DSC05933.webp',
        '/images/play-for-art/DSC06093.webp',
        '/images/play-for-art/DSC06445.webp',
        '/images/play-for-art/DSC06480.webp',
        '/images/play-for-art/DSC06534(1).webp',
        '/images/play-for-art/DSC06561.webp',
        '/images/play-for-art/DSC06606.webp',
        '/images/play-for-art/DSC06646.webp',
        '/images/play-for-art/DSC06673.webp',
        '/images/play-for-art/DSC06687.webp',
        '/images/play-for-art/DSC06748.webp',
        '/images/play-for-art/DSC06837.webp',
        '/images/play-for-art/DSC06866.webp',
      ],
    },

    {
      titulo: "drogas-meditacion",
      images: [
        "images/drogas-meditacion/4.webp",
        "images/drogas-meditacion/3.webp",
        "images/drogas-meditacion/1.webp",
        "images/drogas-meditacion/2.webp",
      ]
    },

{
  titulo: "It's All About Vanishment",
  categoria: 'screenplay',
  media: ["https://vimeo.com/437936022"],
  descripcion: `
    <p><strong>It’s All About Vanishment</strong></p>
    <p>Cuando el dolor irrumpe, ya nada vuelve a ser lo mismo.</p>
    <p>La desaparición no es solo ausencia: es una forma de habitar el vacío.</p>
    <p>Un cuerpo que se exhibe mientras se esfuma, atrapado entre la belleza, el artificio y el hastío.</p>
    <p>Coreografía del duelo, del exceso, del silencio que queda cuando todo lo demás se ha ido.</p>
  `,
},

    {
      titulo: 'Dressed by MM',
      categoria: 'estilismo',
      descripcion: 'Proyecto de estilismo y dirección de arte bajo la marca María Magdalena.',
      media: [
        '/images/dressed-by-mm/Princess Nokia.webp',
        '/images/dressed-by-mm/Bad Gyal.webp',
         '/images/dressed-by-mm/Maria Jose Llergo.webp',
        '/images/dressed-by-mm/Aliyah Morales.webp',
        '/images/dressed-by-mm/Ana Rujas.webp',
        '/images/dressed-by-mm/Brisa Fenoy.webp',
        '/images/dressed-by-mm/Campaña Rives.webp',
        '/images/dressed-by-mm/Campaña Rives Man.webp',
        '/images/dressed-by-mm/Eduardo Casanova.webp',
        '/images/dressed-by-mm/Juancho Marques.webp',
        '/images/dressed-by-mm/Natalia Lacunza.webp',
        '/images/dressed-by-mm/Miss Nina.webp',
        
      ],
    },

    {
      titulo: "Winter-Series",
      images: [
        "images/winter-series/a.webp",
        "images/winter-series/b.webp",
        "images/winter-series/c.webp",
        "images/winter-series/d.webp"
      ]
    },


    {
      titulo: 'The Shame of Spain',
      categoria: 'marca',
      descripcion:
        'Statement visual sobre sexualización, vergüenza y símbolos impuestos. Colores nacionales como herida.',
      copy:
        'The Shame of Spain es un statement visual y emocional... Feminismo, rabia y estética como forma de resistencia.',
      media: [
        '/images/the-shame-of-spain/IMG_0091.webp',
        '/images/the-shame-of-spain/IMG_0135.webp',
        '/images/the-shame-of-spain/IMG_0143.webp',
        '/images/the-shame-of-spain/IMG_0148.webp',
        '/images/the-shame-of-spain/IMG_0167.webp',
      ],
    },


    // MARÍA MAGDALENA
    {
      titulo: 'We are cattle',
      categoria: 'marca',
      descripcion:
        'Ciudadano ≈ ganado: crítica a la persuasión social y pérdida de identidad. Teatro textil como llamada al despertar.',
      copy: `El hombre dentro de la sociedad como individuo que hay que idiotizar... “We are cattle” es una representación irónica y surrealista del panorama. El aspecto teatral de las prendas pretende transmitir de manera impactante un mensaje: necesidad de despertar.`,
      año: undefined,
    },
    {
      titulo: 'Integración',
      categoria: 'marca',
      descripcion:
        'Colección que mezcla opuestos: lo puro y lo caótico, lo tradicional y lo espontáneo. Sevilla 90s/2000s como atmósfera.',
      copy: `Integración es una colección de ropa que mezcla opuestos... La nostalgia funciona como herramienta de integración...`,
      año: undefined,
    },
    { titulo: 'Fashion film “We are cattle”', categoria: 'marca' },
    { titulo: 'Fashion film “Integración”', categoria: 'marca' },
    {
      titulo: 'The Shame of Spain',
      categoria: 'marca',
      descripcion:
        'Statement visual sobre sexualización, vergüenza y símbolos impuestos. Colores nacionales como herida.',
      copy:
        'The Shame of Spain es un statement visual y emocional... Feminismo, rabia y estética como forma de resistencia.',
    },
    {
      titulo: 'Drugs and Meditation',
      categoria: 'marca',
      descripcion:
        'Tensión entre viaje interior y huida exterior; silencio vs. anestesia. Dos respuestas al vacío.',
      copy:
        'Una tensión entre dos extremos... Dos formas de estar —o de no estar— en el mundo.',
    },
    // SCREENPLAY
    {
      titulo: "It's All About Vanishment (Teresa Rofer)",
      categoria: 'screenplay',
      descripcion:
        'Coreografía del duelo, del exceso y del silencio. Cuerpo que se exhibe mientras se esfuma.',
      copy:
        'Cuando el dolor irrumpe... La desaparición no es solo ausencia... atrapado entre la belleza y el hastío.',
    },
    // PRODUCTO
    { titulo: 'Play for Art (Adidas x Juancho Marqués)', categoria: 'producto' },
    { titulo: 'Winter Series (Juancho Marqués)', categoria: 'producto' },
    // VIDEOCLIPS
    { titulo: 'Control (Recycled J & Rels B)', categoria: 'videoclips' },
    { titulo: 'tú y yo (La Zowi)', categoria: 'videoclips' },
    { titulo: 'A tu lado (Machete)', categoria: 'videoclips' },
    // ESTILISMO/ARTE
    { titulo: 'Los ojos del nativo (Juancho Marqués)', categoria: 'estilismo' },
    { titulo: 'Nuevo mundo (Juancho Marqués & Fuel Fandango)', categoria: 'estilismo' },
    { titulo: 'te acuerdas que (Juancho Marqués & Daniela Garsal)', categoria: 'estilismo' },
    { titulo: 'Vuelvo a la nena (Soto Asa)', categoria: 'estilismo' },
  ],

  prensa: [
    { 
      medio: 'Harper’s Bazaar', 
      url: 'https://www.harpersbazaar.com/es/moda/tendencias/g251495/jovenes-disenadores-marcas-espana/?slide=13',
      image: '/images/prensa/prensa1.png'
    },
    { 
      medio: 'El País', 
      url: 'https://elpais.com/elpais/2016/02/23/album/1456241604_001039.html#',
      image: '/images/prensa/prensa2.png'
    },
    { 
      medio: 'Grazia', 
      url: 'https://graziamagazine.com/es/articles/es-la-pasarela-ego-un-verdadero-relevo-generacional/',
      image: '/images/prensa/prensa3.png'
    },
    { 
      medio: 'L’Officiel', 
      url: 'https://www.lofficiel.es/moda/samsung-ego-maria-magdalena',
      image: '/images/prensa/prensa4.png'
    },
    { 
      medio: 'Telva', 
      url: 'https://www.telva.com/estilo-vida/album/2017/03/07/58bd4b67268e3e247e8b4571_22.html',
      image: '/images/prensa/prensa5.png'
    },
    { 
      medio: 'Neo2', 
      url: 'https://www.neo2.com/entrevista-con-maria-magdalena/',
      image: '/images/prensa/prensa6.png'
    },
    { 
      medio: 'Smoda', 
      url: 'https://elpais.com/smoda/moda/la-ley-de-la-calle.html',
      image: '/images/prensa/prensa7.png'
    },
    { medio: 'ICON', 
      url: 'https://elpais.com/elpais/2020/06/03/icon/1591205859_965050.html',
      image: '/images/prensa/prensa8.png'
    },
  
  ],

  cv: {
    experiencia: [
      'Estilismo, diseño e imagen — Juancho Marqués (2020–2022)',
      'Diseño de producto — Adidas (2021)',
      'Fundadora — María Magdalena (2015–2019)',
      'Diseñadora en prácticas — Roberto Diz (2013)',
    ],
    estudios: [
      'Formación en astrología psicológica y evolutiva (Astroterapéutica), 2023',
      'Diseño y Gestión de la Moda (Ceade Leonardo), 2011–2014',
    ],
    premios: [
      'Primer premio Desencaja «Jóvenes diseñadores» — Andalucía de moda',
      'Premio New Designers Awards Neo2 by Sancal',
      'Mejor Cortometraje internacional “We are cattle” — México Fashion Film Festival',
      'Primer premio Fashion Film “Integración” — Madrid Fashion Film Festival',
    ],
  },


}

/* ==== PATCH: actualizar solo descripciones sin tocar imágenes/medias ==== */
const __norm = (s) =>
  (s || '')
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '');


const __descs = {
  // ——— Ya existentes ———
  'maría magdalena': `
    María Magdalena fue una marca de moda conceptual que se mueve entre lo religioso, lo filosófico y lo urbano. Su universo estético está profundamente influenciado por la iconografía cristiana, el folclore andaluz, la cultura popular y el streetwear, con una mirada crítica y poética a la vez.
    La marca propone un discurso dual: lo sagrado y lo profano, lo marginal y lo aspiracional, la devoción y la rebeldía, conviven en cada prenda como parte de una narrativa más amplia.
    Su nombre no es casual: María Magdalena representa el arquetipo femenino contradictorio, la santa y la pecadora, y funciona como símbolo de la complejidad identitaria que la marca explora. Las prendas no son solo moda, sino vehículos para comunicar ideas, muchas veces con una carga irónica, subversiva o política.
    María Magdalena también se posiciona desde una ética de producción consciente, trabajando con talleres locales y apostando por la sostenibilidad y la inclusión social. Más allá del diseño, se presenta como un proyecto artístico y filosófico que utiliza la ropa como lenguaje para cuestionar y narrar.
  `,
  'the shame of spain': `
    The Shame of Spain es un statement visual y emocional. Una pieza crítica sobre la sexualización, la vergüenza de existir y el peso de los símbolos que nos fueron impuestos.
    Colores nacionales usados como herida, como espejo. Un drop que incomoda desde el cuerpo, habla desde el género y subvierte desde la ironía. Feminismo, rabia y estética como forma de resistencia.
  `,
  'drogas-meditacion': `
    Una tensión entre dos extremos: el viaje hacia dentro o la huida hacia fuera. Por un lado, el silencio oriental que enseña a mirar el ego de frente; por el otro, la velocidad occidental que lo anestesia con químicos.
    Dos formas de estar —o de no estar— en el mundo. Dos respuestas al mismo vacío.
  `,
  'Play for Art (Adidas x Juancho Marqués)': `
    Drop Play for Art x Adidas – Dirección Creativa y Concepto
    En este proyecto desarrollé y materialicé el <em>concepto original Play for Art, un universo creativo que mezcla arte, música y moda, donde la expresión y el juego son protagonistas. Sobre esta base, dirigí y coordiné la colaboración con Adidas, creando la camiseta exclusiva que lleva el espíritu de la marca.
    Diseñamos el falso logo de equipo de fútbol con las iniciales de Juancho Marqués, elemento central que conecta identidad, estética y narrativa. Cada detalle —color, tipografía, símbolo— fue pensado para reflejar autenticidad, innovación y diversión, valores esenciales dePlay for Art.
    El resultado es una pieza que trasciende el merchandising tradicional: no solo es un producto, sino una extensión tangible de un concepto artístico, mostrando cómo la dirección creativa y la conceptualización pueden transformar una prenda en experiencia y en identidad.
  `,
  'winter-series': `
    Winter Series Hoodie – Dirección Creativa y Diseño
    Participé en el diseño de la hoodie de la colección Winter Series, dirigiendo el proceso creativo y seleccionando colores y detalles para asegurar coherencia y estilo.
    La prenda combina comodidad y presencia visual, manteniendo la esencia del concepto original.
  `,
  'we are cattle — vogue': `
    We Are Cattle — Vogue reinterpreta el discurso del film en clave editorial. Siluetas escultóricas, estilismo teatral y encuadres frontales para convertir la crítica social en imágenes icónicas.
    Dirección creativa, estilismo y casting dialogan con una luz dura y una composición limpia que refuerzan la idea de alienación y masa. El resultado: una serie potente, directa y memorable.
  `,
  'integración — vogue': `
    Integración — Vogue traduce la mezcla cani / pijo a fotografía: tejidos nobles junto a denim y deportivas, símbolos religiosos con elementos populares, Sevilla como telón y una paleta cálida que une contradicciones.
    Una editorial que condensa el concepto de la colección en imágenes: choque, cruce y madurez estética sin perder la calle.
    `,
  "it's all about vanishment": `
    <p><strong>It’s All About Vanishment</strong></p>
    <p>Cuando el dolor irrumpe, ya nada vuelve a ser lo mismo.</p>
    <p>La desaparición no es solo ausencia: es una forma de habitar el vacío.</p>
    <p>Un cuerpo que se exhibe mientras se esfuma, atrapado entre la belleza, el artificio y el hastío.</p>
    <p>Coreografía del duelo, del exceso, del silencio que queda cuando todo lo demás se ha ido.</p>
  `,

  "integración — fashion film": `
    <p><strong>Integración</strong> es una colección de ropa que mezcla opuestos. Rechaza lo rígido, lo estéril y lo separado. Reúne los contrastes del imaginario personal y los pone a convivir: lo puro y lo caótico, lo tradicional y lo espontáneo, lo espiritual y lo cotidiano.</p>
    <p>La estética se inspira en la Sevilla de finales de los 90 y principios de los 2000, con guiños a las tribus urbanas que marcaban la época: canis y pijos. Dos estilos enfrentados que aquí se cruzan y se mezclan para crear algo nuevo.</p>
    <p>Los tejidos elegantes se combinan con otros más crudos. Hay referencias al flamenco jondo, a la religión, a la sexualidad y a las diferencias de clase. Lo banal y lo profundo se encuentran en un universo textil pensado, pero sin rigidez.</p>
    <p>La nostalgia funciona como herramienta de integración: entender el pasado, revisar patrones y abrir una etapa de madurez y diversión, en la que estar más cerca de la esencia se vuelve una elección.</p>
  `,

  "we are cattle — fashion film": `
    <p>El hombre dentro de la sociedad como individuo que hay que idiotizar para que contribuya a sus reglas. En <em>We are cattle</em> se compara al ciudadano con el ganado.</p>
    <p>Al haberlo sometido a estrategias —cada vez más sutiles— de persuasión, se le va despojando del pensamiento crítico e identidad propia, alejándolo de los valores esenciales. El resultado es un ser humano convertido en cómplice y víctima de la patocracia.</p>
    <p><em>We are cattle</em> es una representación irónica y surrealista del panorama.</p>
    <p>El aspecto teatral de las prendas pretende transmitir de manera impactante un mensaje: necesidad de despertar.</p>
  `,
};

if (Array.isArray(aj?.proyectos)) {
  aj.proyectos = aj.proyectos.map((p) => {
    const matchKey = Object.keys(__descs).find((k) => __norm(k) === __norm(p.titulo));
    return matchKey ? { ...p, descripcion: __descs[matchKey] } : p;
  });
}
/* ==== /PATCH ==== */
